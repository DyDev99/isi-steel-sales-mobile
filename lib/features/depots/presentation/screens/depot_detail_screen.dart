...
class DepotDetailScreen extends StatelessWidget {
  const DepotDetailScreen({super.key, required this.depotId});

  static const String routeName = 'depot-detail';
  final String depotId;

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => sl<DepotDetailCubit>()..load(depotId),
      child: const _DepotDetailView(),
    );
  }
}

class _DepotDetailView extends StatefulWidget {
  const _DepotDetailView();

  @override
  State<_DepotDetailView> createState() => _DepotDetailViewState();
}

class _DepotDetailViewState extends State<_DepotDetailView> {
    ...
    return LocalizedBuilder(
      builder: (context) {
        ...
        return Scaffold(
          ...
          body: SafeArea(
            child: BlocBuilder<DepotDetailCubit, DepotDetailState>(
              builder: (context, state) {
                return switch (state) {
                  DepotDetailLoaded() => ResponsiveContentFrame(
                            ...
                            Expanded(
                              child: TabBarView(
                                children: [
                                  ...
                                  _SalesTab(depot: state.depot),
                                  _PromosTab(depot: state.depot),
                                ],
                              ),
                            ),
                          ],
                  ...
                  DepotDetailError(:final message) => Center(
                      child: Text(message,
...
class _HeroHeaderCardState extends State<_HeroHeaderCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final phoneNum = depot.phone.isNotEmpty ? depot.phone : _fallbackPhone;
    final latitude = depot.hasCoordinates ? depot.latitude : _fallbackLatitude;
    final longitude =
        depot.hasCoordinates ? depot.longitude : _fallbackLongitude;
    final outletTier = (depot.priceGroup?.isNotEmpty ?? false)
        ? depot.priceGroup!
        : _fallbackOutletTier;
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        context.localized(depot.displayName),
                        maxLines: 1,
                      ...
                      Row(
                        children: [
                          ...
                          Text(
                            depot.sapDepotId?.isNotEmpty == true
                                ? depot.sapDepotId!
                                : _fallbackOutletId,
                            style: TextStyle(
...
class _OverviewTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final address = [depot.address, depot.district, depot.province]
        .where((part) => part.trim().isNotEmpty)
        .join(', ');

    final outletId = depot.sapDepotId?.isNotEmpty == true
        ? depot.sapDepotId!
        : (depot.depotCode.isNotEmpty ? depot.depotCode : _fallbackOutletId);
    final outletType = (depot.depotGroup?.isNotEmpty ?? false)
        ? depot.depotGroup!
        : _fallbackOutletType;
    final ownerName =
        depot.ownerName.isNotEmpty ? depot.ownerName : _fallbackOwnerName;
    final phoneNum = depot.phone.isNotEmpty ? depot.phone : _fallbackPhone;
    final telegram = (depot.whatsapp?.isNotEmpty ?? false)
        ? depot.whatsapp!
        : _fallbackTelegram;
      ...
      children: [
        Container(
          ...
          child: Column(
            children: [
              _CompactTile(
                  icon: Icons.tag_rounded, label: 'SAP ID', value: outletId),
              _CompactTile(
              ...
              _CompactTile(
                  icon: Icons.send_rounded, label: 'Telegram', value: telegram),
              _CompactTile(
...
class _SalesTab extends StatelessWidget {
  const _SalesTab({required this.depot});
  final Depot depot;
            ...
            Navigator.of(context).push(
              MaterialPageRoute(
                builder: (_) => OrderHistoryScreen(
                  outletName: context.localized(depot.displayName),
                ),
              ),
            );
          },
...
class _PromosTab extends StatelessWidget {
  const _PromosTab({required this.depot});
  final Depot depot;
