...
class _AddVisitBottomSheetState extends State<AddVisitBottomSheet> {
  ...
  TimeOfDay _plannedTime = TimeOfDay.fromDateTime(DateTime.now().add(const Duration(hours: 1)));
  ...
  Future<void> _pickTime() async {
    final picked = await showTimePicker(context: context, initialTime: _plannedTime);
  ...
  Future<void> _submit() async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Visit to ${_shopNameCtrl.text} added to today\'s route')),
    );
    Navigator.pop(context);
  }

  String get _dateLabel => '${_visitDate.day}/${_visitDate.month}/${_visitDate.year}';
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Add My Visit', style: TextStyle(color: Vibe.text, fontSize: 20.sp, fontWeight: FontWeight.w900)),
          Text('Add a stop to today\'s route', style: TextStyle(color: Vibe.text.withValues(alpha: 0.5), fontSize: 12.sp)),
          SizedBox(height: 20.h),
          if (_submitting)
            const Center(child: Padding(padding: EdgeInsets.all(40), child: CircularProgressIndicator(color: Vibe.violet)))
          else ...[
            Form(
              key: _formKey,
              child: Column(
                ...
                children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            _buildPickerTile(icon: Icons.calendar_today_rounded, label: _dateLabel, onTap: _pickDate),
                          ],
                        ),
                      ),
                      SizedBox(width: 12.w),
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            _buildPickerTile(icon: Icons.schedule_rounded, label: _timeLabel, onTap: _pickTime),
                          ],
                        ),
                      ),
                    ],
                  ...
                  _buildTextField(_notesCtrl, 'Purpose of visit, reminders…', required: false, maxLines: 3),
                ],
              ),
            ),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                onPressed: _submit,
                child: Text('Add Visit', style: TextStyle(color: Colors.white, fontSize: 15.sp, fontWeight: FontWeight.w900)),
              ),
            ),
          ],
        ],
  ...
  Widget _buildTextField(
    ...
    int maxLines = 1,
      ...
      TextFormField(
        ...
        validator: required ? (v) => (v == null || v.trim().isEmpty) ? 'Field required' : null : null,
  ...
  Widget _buildDropdownField() => Container(
        ...
        child: DropdownButtonHideUnderline(
          child: DropdownButton<TerritoryType>(
            ...
            hint: Text('Pick one…', style: TextStyle(color: Vibe.text.withValues(alpha: 0.3), fontSize: 13.sp)),
            dropdownColor: Vibe.bgSoft,
            icon: const Icon(Icons.keyboard_arrow_down_rounded, color: Vibe.text),
            isExpanded: true,
            items: TerritoryType.values
                .map((t) => DropdownMenuItem(value: t, child: Text(t.label, style: const TextStyle(color: Vibe.text))))
                .toList(),
  ...
  Widget _buildPickerTile({required IconData icon, required String label, required VoidCallback onTap}) => InkWell(
        onTap: onTap,
        ...
        child: Container(
          ...
          child: Row(
            children: [
              ...
              SizedBox(width: 8.w),
              Expanded(child: Text(label, style: TextStyle(color: Vibe.text, fontSize: 13.sp, fontWeight: FontWeight.w700))),
            ],
          ),
        ),
      );
