...
class MockLeadData {
  ...
  static const _namePrefixes = [
    'Angkor', 'Mekong', 'Tonle', 'Bayon', 'Naga', 'Golden', 'Royal', 'Victory',
    'Chenla', 'Independence', 'Friendship', 'Kampuchea', 'Sunrise', 'Diamond',
    'Union', 'Prosperity', 'Rithisen', 'Mekong Delta', 'Riverside', 'Highland',
  ];

  static const _nameSuffixes = [
    'Steel Depot', 'Steel Trading', 'Iron Supply', 'Metal Depot', 'Steel Center',
    'Construction Supply', 'Hardware', 'Steel Co.', 'Steel Group',
    'Building Materials', 'Metal Works', 'Steel Warehouse',
  ];

  static const _ownerNames = [
    'Sokha', 'Dara', 'Sopheak', 'Chan', 'Vanna', 'Rithy', 'Sophea', 'Bopha',
    'Chenda', 'Pisey', 'Vuthy', 'Kunthea', 'Sovannaroth', 'Rathanak', 'Malis',
    'Sreymom', 'Pheakdey', 'Sarun', 'Kolab', 'Panha',
  ];
  ...
  static const _products = [
    'Rebar', 'Steel Coil', 'Wire Mesh', 'Galvanized Sheet', 'Steel Pipe',
    'Angle Bar', 'H-Beam', 'Roofing Sheet', 'Steel Nails', 'Binding Wire',
  ];

  // province -> (district list, approx center lat/lng)
  static const _provinces = <String, (List<String>, double, double)>{
    'Phnom Penh': (['Chamkarmon', 'Toul Kork', 'Meanchey', 'Sen Sok', 'Daun Penh'], 11.5564, 104.9282),
  ...
  static Lead _buildLead(Random rand, int index) {
    ...
    final provinceEntry = _provinces.entries.elementAt(rand.nextInt(_provinces.length));
    ...
    final createdDate = DateTime.now().subtract(Duration(days: 5 + rand.nextInt(365)));
    ...
    final leadSource = LeadSource.values[rand.nextInt(LeadSource.values.length)];
    ...
    final contacts = [
      ...
      if (rand.nextBool())
        Contact(
          name: '${rand.nextBool() ? 'Mr.' : 'Ms.'} ${_pick(rand, _ownerNames)}',
    ...
    final interestedProducts =
        List.generate(1 + rand.nextInt(2), (_) => _pick(rand, _products)).toSet().toList();
    ...
    final opportunityInfo = stage == PipelineStage.leads
        ...
        : OpportunityInfo(
            ...
            subStage: stage == PipelineStage.won
                ...
                : OpportunitySubStage.values[rand.nextInt(OpportunitySubStage.values.length)],
            expectedClosingDate: createdDate.add(Duration(days: 20 + rand.nextInt(60))),
            tonnage: rand.nextBool() ? (5 + rand.nextInt(60)).toDouble() : null,
            productGrade: rand.nextBool() ? 'Grade ${_pick(rand, [
                    '40',
                    '60',
                  ])}' : null,
            ...
            productsInterested: List.generate(1 + rand.nextInt(3), (_) => _pick(rand, _products)).toSet().toList(),
            lastContact: DateTime.now().subtract(Duration(days: rand.nextInt(14))),
          );
    ...
    final wonInfo = stage != PipelineStage.won
        ...
        : WonInfo(
            ...
            deliveryTimeline: _pick(rand, ['This month', 'Next month', 'This quarter']),
            onboardingStatus: OnboardingStatus.values[rand.nextInt(OnboardingStatus.values.length)],
            ...
            approvalDate: createdDate.add(Duration(days: 30 + rand.nextInt(30))),
            contractDate: createdDate.add(Duration(days: 35 + rand.nextInt(30))),
            annualRevenue: currentRevenue,
            productsPurchased: List.generate(2 + rand.nextInt(3), (_) => _pick(rand, _products)).toSet().toList(),
            firstOrderDate: createdDate.add(Duration(days: 40 + rand.nextInt(40))),
    ...
    if (stage == PipelineStage.won) {
      docs.addAll([
        ...
        LeadDocument(
          ...
          url: 'https://images.unsplash.com/photo-1553413077-190dd305871c?auto=format&fit=crop&w=800&q=60',
  ...
  static List<ActivityLogItem> buildActivity(Lead lead) {
    final items = <ActivityLogItem>[
      ...
      ActivityLogItem(
        ...
        title: 'Captured GPS',
        description: 'Location pinned at ${lead.latitude.toStringAsFixed(4)}, ${lead.longitude.toStringAsFixed(4)}.',
        timestamp: lead.createdDate.add(const Duration(days: 1)),
        actor: lead.assignedRepName,
      ),
      ActivityLogItem(
    ...
    if (lead.stage == PipelineStage.won) {
      items.addAll([
        ActivityLogItem(
          ...
          description: 'Credit limit of \$${lead.creditLimit.toStringAsFixed(0)} approved.',
        ...
        ActivityLogItem(
          ...
          description: 'Customer record ${lead.wonInfo?.sapCustomerId} created.',
  ...
  static List<NotificationItem> buildNotifications(List<Lead> leads) {
    ...
    final leadsOnly = leads.where((l) => l.stage == PipelineStage.leads).take(3);
    ...
    final opps = leads.where((l) => l.stage == PipelineStage.opportunities).take(3);
    ...
    if (leads.isNotEmpty) {
      items.add(NotificationItem(
        ...
        body: '${leads.first.companyName} has a follow-up scheduled for tomorrow.',
  ...
  static String _pick(Random rand, List<String> list) => list[rand.nextInt(list.length)];
