...
class CartCubit extends Cubit<CartState> {
  ...
  final CheckoutCart _checkoutCart;
  ...
  Future<void> addProduct(Product product, {double quantity = 1, String? leadId}) async {
    CartItem? existing;
    for (final item in _items) {
      if (item.product.id == product.id && item.leadId == leadId) existing = item;
    }
  ...
  Future<PendingOrder?> checkout({String? leadId}) async {
    final result = await _checkoutCart(CheckoutParams(leadId: leadId));
    return result.when(
      success: (order) {
