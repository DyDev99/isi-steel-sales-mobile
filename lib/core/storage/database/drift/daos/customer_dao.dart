...
class CustomerDao extends DatabaseAccessor<AppDatabase>
  ...
  Stream<List<Customer>> watchByTerritory(String territory) {
    return (select(customers)
          ..where((t) =>
              t.territory.equals(territory) & t.deleted.equals(false))
    ...
    await transaction(() async {
      for (final record in records) {
        ...
        await into(customers)
            .insert(record.customer, onConflict: DoUpdate((_) => record.customer));
        await (delete(customerContacts)
  ...
  Future<List<Customer>> fetchFavorites() {
    final query = select(customerFavorites).join([
      innerJoin(customers, customers.id.equalsExp(customerFavorites.customerId)),
    ])
