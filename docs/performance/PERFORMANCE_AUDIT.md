...
> high-blast-radius changes that belong in their own reviewed PRs per `AI_ENGINEERING_PLAYBOOK.md` §3/§8).
...
4. The **truly blank** white screen in the original screenshots was the earlier `SqliteException(26)` DB-corruption (already fixed by the open/self-heal rewrite); after that fix it degraded to the dead empty state — still wrong per `OFFLINE_FIRST.md` §1 and the issue's own requirement: *"If local data does not exist: show loading, synchronize, cache locally, display products. Never display an empty white screen."*
...
- [`depot_stock_count_screen.dart`](../../lib/features/my_visits/presentation/screens/depot_stock_count_screen.dart) — `syncing` renders the loading skeleton with a caption; empty state now takes a message + Retry.
...
integration test per `OFFLINE_FIRST.md` §6. (Depot Stock had **no** test before — adding one is a follow-up.)
...
## 5. Offline-first validation (against `OFFLINE_FIRST.md` §4)
...
high-bar defect (`AI_ENGINEERING_PLAYBOOK.md` §3) and a **data-migration correctness** risk on real
upgrades, not just tests. **Recommend a dedicated fix PR** with a migration test proving upgrade
idempotency (`DATABASE_GUIDE.md` §5). Out of scope for this Depot Stock task, but flagged rather than
silently ignored (`ENGINEERING_STANDARD.md` §11).
