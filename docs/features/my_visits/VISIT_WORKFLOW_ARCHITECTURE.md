...
> Rule honored: **plan before code** (`ENGINEERING_STANDARD.md` §2 — module-by-module, never ahead of
...
| Check-In | ✅ Built | `route_check_in_screen.dart` (perf notes in `docs/performance/PERFORMANCE_AUDIT.md` §2) |
...
- **Reads are local-first** (Drift); route/visit/stock are offline-capture with push telemetry
  (`OFFLINE_FIRST.md` §4). No screen blocks on SAP.
- **WorkflowSession**: `currentScreen` + JSON `navigationArguments` → resume registry rebuilds the exact screen;
  falls back to guided Choose-Stop on any missing/invalid arg (defensive, per `OFFLINE_FIRST.md` §3.2).
...
- **Depot products sometimes empty** → root-caused and **fixed** last pass: the local catalog was empty and the
  ...
  products, or an actionable offline-empty with Retry. Full write-up: `docs/performance/PERFORMANCE_AUDIT.md` §1.
...
- Check-in perf refactor (skeleton-first + progressive GPS/permission overlays) — `PERFORMANCE_AUDIT.md` §2.
- Startup: move legacy import off the pre-`runApp` path — `PERFORMANCE_AUDIT.md` §3.
- `cart_items.customization_json` duplicate-column migration bug — `PERFORMANCE_AUDIT.md` §6.
