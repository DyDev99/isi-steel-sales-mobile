...
> `ENGINEERING_STANDARD.md` (cross-cutting rules), `ARCHITECTURE.md` (layering),
> `SECURITY.md` (threat model), `OFFLINE_FIRST.md`, `SYNC_ENGINE.md`,
> `AI_ENGINEERING_PLAYBOOK.md` (worked example).
...
| When | Do this |
|---|---|
| **Before writing code** | Read §1. Run `graphify affected "<symbol>" --depth 1` for blast radius. Confirm the infrastructure you depend on exists (`MIGRATION_PLAN.md`) — do not build ahead of it. |
...
- **FS-NN-6** — Domain layer imports no Flutter, Drift, or `dio` types
  (`ARCHITECTURE.md` §2, ADR-003). Responsive/UI logic never leaves
  presentation.
...
Full model in `SECURITY.md`. Gate:
...
- **FS-SEC-4** — No hand-rolled cryptography. Use the composite key scheme in
  `DATABASE_GUIDE.md` §2 and platform primitives only.
...
- **FS-MNT-7** — When code and docs disagree, **fix or flag the doc in the same
  PR** (`ENGINEERING_STANDARD.md` §11). Stale docs actively mislead both humans
  and AI agents.
