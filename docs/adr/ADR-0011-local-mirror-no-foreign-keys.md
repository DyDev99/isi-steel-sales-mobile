...
- **Related**: ADR-002 (offline-first), ADR-004 (DAO pattern), ADR-006 (sync engine), `docs/blueprint/local-storage-architecture.md` §3, `docs/feature/my-visits/api.md` §5, `docs/feature/customer/api.md`
...
`docs/feature/customer/api.md` documents scoping that makes a
...
1. **`route_customers`** — a new flat table holding the customer rows the route
   feed already sends (`docs/feature/my-visits/api.md` §7.1 `CustomerStopInfo`). Route
