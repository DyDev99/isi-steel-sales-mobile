...
- **Related**: `flutter-web-support.md` §3/§10, `SECURITY.md` §3/§4, `ARCHITECTURE.md` §3, ADR-001, ADR-002, ADR-008

---

## Context

`flutter-web-support.md` establishes that the presentation and domain layers of this app are web-ready, and that the blocker to a Flutter Web target sits entirely in `core/` persistence. The specific problem:

`ARCHITECTURE.md` §3 assigns every byte of application state to one of four stores, and three of them have no faithful web equivalent.
...
1. **`ARCHITECTURE.md` §3 Layer 1 is amended for the web target only**: business data lives in an *ephemeral* Drift database. Encryption at rest is not required because there is no rest — the requirement is satisfied by absence of persistence, not by a weaker cipher.
...
5. **Layer 3 (secrets) on web is explicitly weaker** and is handled separately under `flutter-web-support.md` §4/W6 — short-lived in-memory access tokens, strict CSP, and a compile-time guard ensuring `Env.dbSalt` and SAP credentials never reach a web bundle. **Envied obfuscation provides no protection in a JS/WASM bundle** and must not be relied on there.

Offline capability on web is therefore **session-scoped**: a rep can work disconnected for the length of a browser session, and unsynced work is lost if the tab closes while offline. This limitation must be stated honestly in `OFFLINE_FIRST.md` and surfaced in the web UI — it must not be presented as parity with mobile.
...
- **No plaintext PII at rest on any platform.** The web target cannot reintroduce the highest-severity finding in `MIGRATION_PLAN.md` §9's risk register, because it writes nothing to disk.
...
- **No new cryptography.** Nothing to design, review, or get wrong. `SECURITY.md` §4's "never implement custom cryptography" rule is satisfied trivially.
- **Not blocked on `sap_client.dart`.** That file is still a 0-byte stub (`MIGRATION_PLAN.md` Phase 4/7); an online-only web build would have pulled the entire SAP gateway onto the web critical path.
...
- **Cold start on web re-pulls data**, since nothing is cached across sessions. This makes first-load latency a real UX concern on a Cambodian field connection and interacts with the bundle-size budget (`flutter-web-support.md` R5/W7).
- **Two executor paths to keep honest.** Divergence between the native and in-memory connections is a live risk (R3). Mitigation is mandatory and non-negotiable: the DAO test suite runs against **both** executors, per `flutter-web-support.md` §8.
...
- **`ARCHITECTURE.md` §3's four-layer matrix now has a documented platform exception**, which is a small ongoing comprehension cost for new engineers. Mitigated by this ADR being linked from that section.
...
- **Option C — persistent encrypted WASM database (`sqlite3mc`).** The only option offering true persistent offline on web. Rejected because **there is nowhere safe to put the key.** Browsers have no hardware-backed keystore; the key would live in `localStorage`/IndexedDB alongside the data it protects, reachable by any XSS on the origin. That is obfuscation presented as encryption — the most dangerous outcome of the three, because it would let the app claim `SECURITY.md` §3 compliance it does not have. It would also require superseding ADR-008, re-proving fail-closed behaviour on a second cipher stack, and maintaining two on-disk formats. Revisit only if a hardware-backed browser key primitive becomes broadly available *and* persistent web offline is shown to be a real field requirement.
