...
|---|---|---|---|
| `GET` | `/mobile/depots` | `depots.read` | Paged list + sync metadata |
| `GET` | `/mobile/depots/{id}` | `depots.read` | Full depot + contacts |
| `POST` | `/mobile/depots` | `depots.create` | Created depot (201) |
| `PUT` | `/mobile/depots/{id}` | `depots.update` | Updated depot |
| `DELETE` | `/mobile/depots/{id}` | `depots.delete` | 204 |
| `POST` | `/mobile/depots/business-partner` | `depots.create` | SAP registration status |
| `GET` | `/depots/by-code/{code}` | `depots.read` | One depot, fetched from SAP if unknown |

The last two are new. See [Registering a business partner for SAP](#registering-a-business-partner-for-sap)
and [Looking a depot up by code](#looking-a-depot-up-by-code).
...
- [Authentication-guild-integratemobile.md](../authentication/integration-guide.md) — sign-in, tokens, refresh
- [MobileDepotApi.md](api-design-notes.md) — server-side design decisions and migration notes
- `DepotSapIntegration.md` (⧉ backend repo) — what happens after you submit: the
