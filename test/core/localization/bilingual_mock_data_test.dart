...
void main() {
  ...
  group('lead pipeline', () {
    final leads = MockLeadData.generate();

    test('every lead carries a Khmer company name', () {
      final missing =
          leads.where((l) => l.companyNameKh.trim().isEmpty).toList();
      expect(missing, isEmpty);
    });

    test('displayName resolves per language', () {
      for (final l in leads) {
        expect(l.displayName.resolve('en'), isNotEmpty);
        expect(l.displayName.resolve('km'), isNotEmpty);
      }
    });
  });
