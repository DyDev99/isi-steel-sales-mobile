...
void main() {
  ...
  setUp(() {
    ...
    when(() => remote.logout(accessToken: any(named: 'accessToken')))
        .thenAnswer((_) async {});
  });
  ...
  test('clears local state even with no connectivity', () async {
    ...
    verify(local.clear).called(1);
    verifyNever(() => remote.logout(accessToken: any(named: 'accessToken')));
  });
  ...
  test('revokes with the outgoing token, read before the store is cleared',
    ...
    verifyInOrder([
      () => local.readToken(),
      local.clear,
      () => remote.logout(accessToken: token.accessToken),
    ]);
  });
